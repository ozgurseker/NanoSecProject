...
df_short <- tibble(time = unique(df$time)) %>% left_join(df_sudden) %>% select(time, sudden_bid_change_s) %>%
  mutate(counter = cumsum(!is.na(sudden_bid_change_s))) %>% filter(counter > lag(counter, 10)) %>% 
  select(time) %>% left_join(df) %>% left_join(df_sudden)
...
df_shorter <- df_shorter[order(nrow(df_shorter):1),]
df_shorter <- df_shorter %>% fill(last_time, .direction = "down") %>% filter(time < last_time)

df_shorter <- df_shorter %>% mutate(time_diff = time - lag(time)) %>%
  mutate(
...
df_returns <- df_shorter %>% filter(!is.na(price_change_s), !is.na(price_change_f)) %>%
  tbl_time(index = time) %>% 
  collapse_by(period = "10 ms", side = "start") %>%
  group_by(time) %>% 
...
write_csv(df_returns, "returns.csv")
